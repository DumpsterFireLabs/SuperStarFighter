...
static func _validate_production_screens(context: TestContext, tree_parent: Node) -> void:
	...
	context.expect_equal(client.connection_controller.lobby_options_button.theme_type_variation, &"SecondaryButton", "secondary lobby action uses the shared semantic action language")
	...
	var host_error: Error = client.connection_controller._start_hosted_server({"port": 17459, "max_players": 32, "rounds_to_win": 3, "server_name": "Embedded Test Arena", "lobby_password": "test-lobby"})
	...
	context.expect_true(client.connection_controller._hosted_server_bridge.role == NetworkBridge.Role.SERVER and client.connection_controller._hosted_server_multiplayer != client.multiplayer, "hosted server and playable client retain independent MultiplayerAPI instances")
	...
	context.expect_true(client.connection_controller._hosted_server_root != null, "opening the F2 confirmation cannot stop the hosted server")
	...
	client.connection_controller._stop_hosted_server()
	var reserved_host_error: Error = client.connection_controller._start_hosted_server({"port": LanDiscoveryProtocol.DISCOVERY_PORT, "max_players": 32, "rounds_to_win": 3, "server_name": "Collision Test", "lobby_password": "test-lobby"})
	...
	client.connection_controller._stop_hosted_server()
	context.expect_true(client.connection_controller.lobby_panel != null, "production lobby screen exists")
	context.expect_true(client.connection_controller.lobby_settings_button != null and client.connection_controller.lobby_settings_button.pressed.is_connected(client._show_settings.bind(false)), "waiting lobby Settings button opens the shared player settings screen")
	...
	context.expect_true(client.connection_controller.lobby_disconnect_button != null and client.connection_controller.lobby_disconnect_button.pressed.is_connected(client._disconnect_online), "waiting lobby Disconnect button is wired to the shared disconnect action")
	...
	context.expect_true(client.connection_controller.lobby_options_popup != null and client.connection_controller.powerups_button != null, "lobby exposes a dedicated match options menu")
	context.expect_equal(client.connection_controller.lobby_options_button.text, "MATCH SETUP", "ship colour is no longer presented as a separate lobby option")
	context.expect_false(client.connection_controller.powerups_button.button_pressed, "random spawn powerups are visibly disabled by default")
	context.expect_approx(client.connection_controller.powerup_interval_control.value, 20.0, "random drop interval visibly defaults to twenty seconds")
	context.expect_false(client.connection_controller.powerups_permanent_button.button_pressed, "random drop permanence visibly defaults off")
	context.expect_approx(client.connection_controller.overtime_start_control.value, 45.0, "overtime visibly defaults to forty-five seconds")
	context.expect_equal(client.connection_controller.game_mode_control.item_count, 5, "lobby options expose all five selectable game modes")
	context.expect_equal(client.connection_controller.game_mode_control.get_selected_id(), GameModeRules.Mode.DEATH_MATCH, "Death Match is visibly selected by default")
	context.expect_true(client.connection_controller.team_count_row != null and client.connection_controller.team_count_control != null, "lobby options provide a configurable Team Death Match team count")
	context.expect_false(client.connection_controller.team_count_row.visible, "team count stays hidden outside Team Death Match")
	context.expect_equal(int(client.connection_controller.team_count_control.min_value), 2, "Team Death Match requires at least two teams")
	context.expect_equal(int(client.connection_controller.team_count_control.max_value), 8, "Team Death Match supports up to eight teams")
	context.expect_equal(client.connection_controller.npc_all_difficulty_control.item_count, 5, "lobby provides one bulk dropdown covering every NPC difficulty")
	...
	context.expect_equal(client.connection_controller.lobby_roster.get_child_count(), 32, "scrollable lobby roster renders all 32 participants")
	context.expect_true(client.connection_controller.lobby_roster.get_parent() is ScrollContainer, "32-player lobby roster is scrollable")
	...
	var remote_color_swatch := client.connection_controller.lobby_roster.get_child(1).get_node("ShipColor") as Button
	...
	context.expect_true(client.connection_controller.ship_color_popup.visible, "clicking the local roster colour opens the colour wheel")
	context.expect_true(client.connection_controller.ship_color_blocker.visible and not client.connection_controller.lobby_panel.visible, "colour selection behaves as a true modal and blocks the lobby beneath it")
	var prior_color: Color = client.connection_controller.preferred_ship_color
	client.connection_controller._on_ship_color_changed(Color("ff4ea3"))
	client.connection_controller._on_ship_pattern_selected(ShipAppearanceScript.PATTERNS.find(ShipAppearanceScript.LEOPARD))
	context.expect_equal(client.connection_controller.preferred_ship_color, prior_color, "wheel changes remain pending until Apply is pressed")
	context.expect_equal(client.connection_controller.pending_ship_color.to_html(false), "ff4ea3", "wheel tracks the pending custom colour")
	context.expect_equal(client.connection_controller.pending_ship_pattern, ShipAppearanceScript.LEOPARD, "pattern selection remains pending until Apply is pressed")
	client.connection_controller._apply_ship_color()
	context.expect_equal(client.connection_controller.preferred_ship_color.to_html(false), "ff4ea3", "Apply commits the selected ship colour")
	context.expect_equal(client.connection_controller.preferred_ship_pattern, ShipAppearanceScript.LEOPARD, "Apply commits the selected hull pattern")
	context.expect_false(client.connection_controller.ship_color_popup.visible, "Apply closes the roster colour picker")
	context.expect_true(not client.connection_controller.ship_color_blocker.visible and client.connection_controller.lobby_panel.visible, "closing colour selection restores the waiting lobby")
	client.connection_controller._show_ship_color_popup()
	client.connection_controller._on_ship_pattern_selected(ShipAppearanceScript.PATTERNS.find(ShipAppearanceScript.ZEBRA))
	client.connection_controller._on_random_color_pressed()
	context.expect_equal(client.connection_controller.preferred_ship_pattern, ShipAppearanceScript.ZEBRA, "Random Colour applies the pending pattern while randomizing only the colour")
	client.connection_controller._show_lobby_options()
	context.expect_true(client.connection_controller.lobby_options_popup.visible and client.connection_controller.lobby_options_blocker.visible and not client.connection_controller.lobby_panel.visible, "match options behaves as a focused modal surface")
	client.connection_controller._hide_lobby_options()
	context.expect_true(not client.connection_controller.lobby_options_blocker.visible and client.connection_controller.lobby_panel.visible, "closing match options restores the waiting lobby")
	client.bridge.session.local_peer_id = 3
	client.connection_controller._rebuild_lobby_roster({"players": players, "leader_id": 2, "match_active": false}, false)
	context.expect_true(not (client.connection_controller.lobby_roster.get_child(1).get_node("ShipColor") as Button).disabled, "a non-leader human can edit their own roster colour")
	context.expect_true((client.connection_controller.lobby_roster.get_child(0).get_node("ShipColor") as Button).disabled, "the non-leader still cannot edit the host's colour")
	client.bridge.session.local_peer_id = 2
	client.connection_controller._rebuild_lobby_roster({"players": players, "leader_id": 2, "match_active": false}, true)
	context.expect_false(client.connection_controller.start_button.disabled, "leader can launch once all humans are ready")
	context.expect_equal(client.connection_controller.start_button.text, "Start Match", "ready multiplayer lobby uses ordinary start wording")
	...
	client._on_lobby_state(client.bridge.latest_lobby_state)
	var difficulty_control := client.connection_controller.lobby_roster.get_child(2).get_node("NpcDifficulty") as OptionButton
	...
	context.expect_equal(client.connection_controller.npc_all_difficulty_control.get_selected_id(), NpcPilotController.Difficulty.INSANE, "bulk NPC dropdown reflects the authoritative lobby setting")
	context.expect_false(client.connection_controller.npc_all_difficulty_control.disabled, "bulk NPC difficulty remains editable for the lobby leader")
	context.expect_approx(client.connection_controller.powerup_interval_control.value, 12.0, "lobby renders the authoritative random drop interval")
	context.expect_true(client.connection_controller.powerups_permanent_button.button_pressed, "lobby renders authoritative drop permanence")
	context.expect_approx(client.connection_controller.overtime_start_control.value, 75.0, "lobby renders the authoritative overtime start")
	context.expect_equal(client.connection_controller.game_mode_control.get_selected_id(), GameModeRules.Mode.TEAM_CAPTURE_THE_FLAG, "lobby renders the authoritative game-mode selection")
	context.expect_true(client.connection_controller.game_mode_note.text.contains("neutral center flag"), "game-mode selection explains its objective")
	context.expect_false(client.connection_controller.team_count_row.visible, "Team Capture the Flag remains a fixed two-team mode")
	context.expect_false((client.connection_controller.lobby_roster.get_child(0).get_node("TeamAssignment") as OptionButton).disabled, "host may assign a human team")
	context.expect_false((client.connection_controller.lobby_roster.get_child(2).get_node("TeamAssignment") as OptionButton).disabled, "host may assign an NPC team")
	client.bridge.session.local_peer_id = 3
	client.connection_controller._rebuild_lobby_roster({"players": configurable_players, "leader_id": 2, "game_mode": GameModeRules.Mode.TEAM_CAPTURE_THE_FLAG, "team_count": 2, "match_active": false}, false)
	context.expect_true((client.connection_controller.lobby_roster.get_child(0).get_node("TeamAssignment") as OptionButton).disabled, "non-host cannot change another human's team")
	context.expect_false((client.connection_controller.lobby_roster.get_child(2).get_node("TeamAssignment") as OptionButton).disabled, "non-host may assign an NPC team")
	context.expect_false((client.connection_controller.lobby_roster.get_child(1).get_node("TeamAssignment") as OptionButton).disabled, "non-host may assign their own team")
	client.bridge.session.local_peer_id = 2
	...
	client._on_lobby_state(client.bridge.latest_lobby_state)
	context.expect_true(client.connection_controller.team_count_row.visible, "Team Death Match reveals the team-count option")
	context.expect_equal(int(client.connection_controller.team_count_control.value), 4, "team-count option reflects authoritative lobby state")
	context.expect_true(client.connection_controller.start_button.disabled and client.connection_controller.start_button.text == "Configure All Teams", "host cannot launch until every configured team is populated")
	...
	client._on_lobby_state(client.bridge.latest_lobby_state)
	context.expect_true(client.connection_controller.start_button.disabled and client.connection_controller.start_button.text == "Enable NPCs to Start Solo", "solo human is directed to enable NPCs")
	...
	var disconnect_host_error: Error = client.connection_controller._start_hosted_server({"port": 17459, "max_players": 32, "rounds_to_win": 3, "server_name": "Disconnect Test Arena", "lobby_password": "test-lobby"})
	...
	client.connection_controller.lobby_panel.visible = true
	client.connection_controller.lobby_disconnect_button.pressed.emit()
	context.expect_true(client.connection_controller.connection_form_panel.visible and not client.connection_controller.lobby_panel.visible, "waiting-lobby Disconnect immediately returns to the connection menu")
	...
	context.expect_true(client.connection_controller._hosted_server_root == null and client.connection_controller._hosted_server_bridge == null, "waiting-lobby Disconnect stops and releases the embedded host")
