...
static func _validate_production_screens(context: TestContext, tree_parent: Node) -> void:
	...
	context.expect_true(client.connection_controller.connection_screen != null, "production connection screen exists")
	...
	context.expect_equal(client.connection_controller.connection_tabs.get_tab_count(), 3, "connection screen separates LAN, direct-connect, and host flows")
	context.expect_true(client.connection_controller.direct_password_field != null and client.connection_controller.direct_password_field.secret, "direct connect requires a masked lobby-password field")
	context.expect_true(client.connection_controller.remember_password_button != null, "direct connect offers client-local remembered passwords by server endpoint")
	context.expect_true(client.connection_controller.host_password_field != null and client.connection_controller.host_password_field.secret, "hosts must set a masked lobby password")
	client.connection_controller.connection_tabs.current_tab = 1
	context.expect_equal(client.connection_controller.host_field.find_next_valid_focus(), client.connection_controller.port_field, "Tab advances from the direct-connect host field to its port field")
	context.expect_equal(client.connection_controller.port_field.find_next_valid_focus(), client.connection_controller.direct_password_field, "Tab advances from the direct-connect port field to its password field")
	context.expect_equal(client.connection_controller.direct_password_field.find_prev_valid_focus(), client.connection_controller.port_field, "Shift+Tab returns from the direct-connect password field to its port field")
	client.connection_controller.connection_tabs.current_tab = 2
	context.expect_equal(client.connection_controller.server_name_field.find_next_valid_focus(), client.connection_controller.host_port_field, "Tab advances from the hosted server name to its port field")
	context.expect_equal(client.connection_controller.host_port_field.find_next_valid_focus(), client.connection_controller.host_password_field, "Tab advances from the hosted server port to its password field")
	client.connection_controller.connection_tabs.current_tab = 0
	context.expect_equal(client.connection_controller.preferences.endpoint_key(" EXAMPLE.COM ", 7000), client.connection_controller.preferences.endpoint_key("example.com", 7000), "remembered-password keys normalize host casing and whitespace")
	context.expect_equal(client.connection_controller.preferences.endpoint_key("[2001:db8::1]", 7000), client.connection_controller.preferences.endpoint_key("2001:db8::1", 7000), "remembered-password keys normalize bracketed IPv6 addresses")
	context.expect_false(client.connection_controller.preferences.endpoint_key("example.com", 7000) == client.connection_controller.preferences.endpoint_key("example.com", 7001), "remembered passwords are isolated by gameplay port")
	context.expect_true(client.connection_controller.lan_browser != null and client.connection_controller.lan_browser.mode == LanDiscoveryService.Mode.BROWSER, "connection screen actively browses for LAN servers")
	...
	client.connection_controller._on_lan_servers_updated(discovered_servers)
	context.expect_equal(client.connection_controller.lan_servers_container.get_child_count(), 2, "LAN browser renders one structured row per discovered server")
	var compatible_join := client.connection_controller.lan_servers_container.get_child(0).get_child(0).get_child(1) as Button
	var incompatible_join := client.connection_controller.lan_servers_container.get_child(1).get_child(0).get_child(1) as Button
	...
	var host_error: Error = client.connection_controller.hosted_session.start({"port": 17459, "max_players": 32, "rounds_to_win": 3, "server_name": "Embedded Test Arena", "lobby_password": "test-lobby"})
	...
	context.expect_true(client.connection_controller.hosted_session.server_bridge.role == NetworkBridge.Role.SERVER and client.connection_controller.hosted_session.server_multiplayer != client.multiplayer, "hosted server and playable client retain independent MultiplayerAPI instances")
	...
	context.expect_true(client.connection_controller.hosted_session.runtime_root != null, "opening the F2 confirmation cannot stop the hosted server")
	...
	client.connection_controller.hosted_session.stop()
	var reserved_host_error: Error = client.connection_controller.hosted_session.start({"port": LanDiscoveryProtocol.DISCOVERY_PORT, "max_players": 32, "rounds_to_win": 3, "server_name": "Collision Test", "lobby_password": "test-lobby"})
	...
	client.connection_controller.hosted_session.stop()
	context.expect_true(client.connection_controller.lobby.lobby_panel != null, "production lobby screen exists")
	context.expect_true(client.connection_controller.lobby.lobby_settings_button != null and client.connection_controller.settings_requested.is_connected(client._show_settings.bind(false)), "waiting lobby Settings button opens the shared player settings screen")
	context.expect_true(client.connection_controller.direct_connect_button != null and client.connection_controller.direct_connect_button.pressed.is_connected(client.connection_controller._connect_online), "Direct Connect button is wired to its connection action")
	context.expect_true(client.connection_controller.host_join_button != null and client.connection_controller.host_join_button.pressed.is_connected(client.connection_controller._host_online), "Host & Join button is wired to its hosting action")
	context.expect_true(client.connection_controller.lobby.lobby_disconnect_button != null and client.connection_controller.disconnect_requested.is_connected(client._disconnect_online), "waiting lobby Disconnect button is wired to the shared disconnect action")
	...
	context.expect_true(client.connection_controller.lobby.lobby_options_popup != null and client.connection_controller.lobby.powerups_button != null, "lobby exposes a dedicated match options menu")
	context.expect_equal(client.connection_controller.lobby.lobby_options_button.text, "MATCH SETUP", "ship colour is no longer presented as a separate lobby option")
	context.expect_false(client.connection_controller.lobby.powerups_button.button_pressed, "random spawn powerups are visibly disabled by default")
	context.expect_approx(client.connection_controller.lobby.powerup_interval_control.value, 20.0, "random drop interval visibly defaults to twenty seconds")
	context.expect_false(client.connection_controller.lobby.powerups_permanent_button.button_pressed, "random drop permanence visibly defaults off")
	context.expect_approx(client.connection_controller.lobby.overtime_start_control.value, 45.0, "overtime visibly defaults to forty-five seconds")
	context.expect_equal(client.connection_controller.lobby.game_mode_control.item_count, 5, "lobby options expose all five selectable game modes")
	context.expect_equal(client.connection_controller.lobby.game_mode_control.get_selected_id(), GameModeRules.Mode.DEATH_MATCH, "Death Match is visibly selected by default")
	context.expect_true(client.connection_controller.lobby.team_count_row != null and client.connection_controller.lobby.team_count_control != null, "lobby options provide a configurable Team Death Match team count")
	context.expect_false(client.connection_controller.lobby.team_count_row.visible, "team count stays hidden outside Team Death Match")
	context.expect_equal(int(client.connection_controller.lobby.team_count_control.min_value), 2, "Team Death Match requires at least two teams")
	context.expect_equal(int(client.connection_controller.lobby.team_count_control.max_value), 8, "Team Death Match supports up to eight teams")
	context.expect_equal(client.connection_controller.lobby.npc_all_difficulty_control.item_count, 5, "lobby provides one bulk dropdown covering every NPC difficulty")
	...
	context.expect_true(client.credits_panel != null and client.connection_controller.credits_button != null, "main menu exposes the dedicated credits screen")
	client.connection_controller.credits_button.pressed.emit()
	context.expect_true(client.credits_panel.visible, "Credits button opens the credits screen through navigation intent")
	...
	context.expect_equal(client.connection_controller.lobby.lobby_roster.get_child_count(), 32, "scrollable lobby roster renders all 32 participants")
	context.expect_true(client.connection_controller.lobby.lobby_roster.get_parent() is ScrollContainer, "32-player lobby roster is scrollable")
	context.expect_true(client.connection_controller.connection_screen.visible and not client.connection_controller.connection_form_panel.visible, "waiting lobby uses the centered menu backdrop instead of the connect form")
	...
	context.expect_true(client.connection_controller.lobby.lobby_panel.visible and not client.settings_controller.settings_panel.visible and not client.settings_return_to_lobby, "Back from player settings restores the waiting lobby")
	context.expect_equal(client.connection_controller.lobby.lobby_roster.get_child(1).get_child_count(), 5, "leader receives colour identity and an eject control for another human")
	context.expect_true(client.connection_controller.lobby.powerups_button.button_pressed and not client.connection_controller.lobby.powerups_button.disabled, "lobby leader sees and can edit the authoritative powerup option")
	var local_color_swatch := client.connection_controller.lobby.lobby_roster.get_child(0).get_node("ShipColor") as Button
	var remote_color_swatch := client.connection_controller.lobby.lobby_roster.get_child(1).get_node("ShipColor") as Button
	...
	context.expect_true(client.connection_controller.lobby.ship_color_popup.visible, "clicking the local roster colour opens the colour wheel")
	context.expect_true(client.connection_controller.lobby.ship_color_blocker.visible and not client.connection_controller.lobby.lobby_panel.visible, "colour selection behaves as a true modal and blocks the lobby beneath it")
	var prior_color: Color = client.connection_controller.lobby.preferred_ship_color
	client.connection_controller.lobby._on_ship_color_changed(Color("ff4ea3"))
	client.connection_controller.lobby._on_ship_pattern_selected(ShipAppearanceScript.PATTERNS.find(ShipAppearanceScript.LEOPARD))
	context.expect_equal(client.connection_controller.lobby.preferred_ship_color, prior_color, "wheel changes remain pending until Apply is pressed")
	context.expect_equal(client.connection_controller.lobby.pending_ship_color.to_html(false), "ff4ea3", "wheel tracks the pending custom colour")
	context.expect_equal(client.connection_controller.lobby.pending_ship_pattern, ShipAppearanceScript.LEOPARD, "pattern selection remains pending until Apply is pressed")
	client.connection_controller.lobby._apply_ship_color()
	context.expect_equal(client.connection_controller.lobby.preferred_ship_color.to_html(false), "ff4ea3", "Apply commits the selected ship colour")
	context.expect_equal(client.connection_controller.lobby.preferred_ship_pattern, ShipAppearanceScript.LEOPARD, "Apply commits the selected hull pattern")
	context.expect_false(client.connection_controller.lobby.ship_color_popup.visible, "Apply closes the roster colour picker")
	context.expect_true(not client.connection_controller.lobby.ship_color_blocker.visible and client.connection_controller.lobby.lobby_panel.visible, "closing colour selection restores the waiting lobby")
	client.connection_controller.lobby._show_ship_color_popup()
	client.connection_controller.lobby._on_ship_pattern_selected(ShipAppearanceScript.PATTERNS.find(ShipAppearanceScript.ZEBRA))
	client.connection_controller.lobby._on_random_color_pressed()
	context.expect_equal(client.connection_controller.lobby.preferred_ship_pattern, ShipAppearanceScript.ZEBRA, "Random Colour applies the pending pattern while randomizing only the colour")
	client.connection_controller.lobby._show_lobby_options()
	context.expect_true(client.connection_controller.lobby.lobby_options_popup.visible and client.connection_controller.lobby.lobby_options_blocker.visible and not client.connection_controller.lobby.lobby_panel.visible, "match options behaves as a focused modal surface")
	client.connection_controller.lobby._hide_lobby_options()
	context.expect_true(not client.connection_controller.lobby.lobby_options_blocker.visible and client.connection_controller.lobby.lobby_panel.visible, "closing match options restores the waiting lobby")
	client.bridge.session.local_peer_id = 3
	client.connection_controller.lobby._rebuild_lobby_roster({"players": players, "leader_id": 2, "match_active": false}, false)
	context.expect_true(not (client.connection_controller.lobby.lobby_roster.get_child(1).get_node("ShipColor") as Button).disabled, "a non-leader human can edit their own roster colour")
	context.expect_true((client.connection_controller.lobby.lobby_roster.get_child(0).get_node("ShipColor") as Button).disabled, "the non-leader still cannot edit the host's colour")
	client.bridge.session.local_peer_id = 2
	...
	client._on_lobby_state(client.bridge.latest_lobby_state)
	var difficulty_control := client.connection_controller.lobby.lobby_roster.get_child(2).get_node("NpcDifficulty") as OptionButton
	...
	context.expect_equal(client.connection_controller.lobby.npc_all_difficulty_control.get_selected_id(), NpcPilotController.Difficulty.INSANE, "bulk NPC dropdown reflects the authoritative lobby setting")
	context.expect_false(client.connection_controller.lobby.npc_all_difficulty_control.disabled, "bulk NPC difficulty remains editable for the lobby leader")
	context.expect_approx(client.connection_controller.lobby.powerup_interval_control.value, 12.0, "lobby renders the authoritative random drop interval")
	context.expect_true(client.connection_controller.lobby.powerups_permanent_button.button_pressed, "lobby renders authoritative drop permanence")
	context.expect_approx(client.connection_controller.lobby.overtime_start_control.value, 75.0, "lobby renders the authoritative overtime start")
	context.expect_equal(client.connection_controller.lobby.game_mode_control.get_selected_id(), GameModeRules.Mode.TEAM_CAPTURE_THE_FLAG, "lobby renders the authoritative game-mode selection")
	context.expect_true(client.connection_controller.lobby.game_mode_note.text.contains("neutral center flag"), "game-mode selection explains its objective")
	context.expect_false(client.connection_controller.lobby.team_count_row.visible, "Team Capture the Flag remains a fixed two-team mode")
	context.expect_false((client.connection_controller.lobby.lobby_roster.get_child(0).get_node("TeamAssignment") as OptionButton).disabled, "host may assign a human team")
	context.expect_false((client.connection_controller.lobby.lobby_roster.get_child(2).get_node("TeamAssignment") as OptionButton).disabled, "host may assign an NPC team")
	client.bridge.session.local_peer_id = 3
	client.connection_controller.lobby._rebuild_lobby_roster({"players": configurable_players, "leader_id": 2, "game_mode": GameModeRules.Mode.TEAM_CAPTURE_THE_FLAG, "team_count": 2, "match_active": false}, false)
	context.expect_true((client.connection_controller.lobby.lobby_roster.get_child(0).get_node("TeamAssignment") as OptionButton).disabled, "non-host cannot change another human's team")
	context.expect_false((client.connection_controller.lobby.lobby_roster.get_child(2).get_node("TeamAssignment") as OptionButton).disabled, "non-host may assign an NPC team")
	context.expect_false((client.connection_controller.lobby.lobby_roster.get_child(1).get_node("TeamAssignment") as OptionButton).disabled, "non-host may assign their own team")
	client.bridge.session.local_peer_id = 2
	...
	context.expect_false(client.connection_controller.lobby.lobby_panel.visible, "lobby menu remains hidden throughout the game loop")
	...
	context.expect_true(client.connection_controller.connection_screen.visible and client.connection_controller.connection_status.text.contains("try again"), "connection rejection returns to a usable recovery screen")
	var disconnect_host_error: Error = client.connection_controller.hosted_session.start({"port": 17459, "max_players": 32, "rounds_to_win": 3, "server_name": "Disconnect Test Arena", "lobby_password": "test-lobby"})
	...
	client.connection_controller.connection_form_panel.visible = false
	client.connection_controller.lobby.lobby_panel.visible = true
	client.connection_controller.lobby.lobby_disconnect_button.pressed.emit()
	context.expect_true(client.connection_controller.connection_form_panel.visible and not client.connection_controller.lobby.lobby_panel.visible, "waiting-lobby Disconnect immediately returns to the connection menu")
	...
	context.expect_true(client.connection_controller.hosted_session.runtime_root == null and client.connection_controller.hosted_session.server_bridge == null, "waiting-lobby Disconnect stops and releases the embedded host")
