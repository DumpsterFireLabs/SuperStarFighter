...
static func _validate_production_screens(context: TestContext, tree_parent: Node) -> void:
	...
	context.expect_equal((offline_kill_entry.get_child(0).get_child(0) as Label).text, "Pilot 1", "offline kill feed resolves the player ship name")
	sandbox._reset_combatants()
	...
	context.expect_equal(client.version_label.text, "BETA 9  ·  VERSION 0.1.0-beta.9", "main screen displays the canonical Beta 9 version")
