...
static func run(context: TestContext, parent: Node) -> void:
	for preset in Presets.PRESETS:
		...
		context.expect_equal(lobby.participant_count(), int(preset.players) if bool(preset.get("npcs", true)) else 1, "preset applies its explicit NPC-fill policy")
		...
		if String(preset.id) != "competitive":
			context.expect_true(lobby.team_setup_error().is_empty(), "casual preset supplies valid teams")
		else:
			context.expect_false(lobby.team_setup_error().is_empty(), "human-only teams wait for opponents")
	...
	lobby.request_player_limit(2, 32)
	lobby.request_game_mode(2, GameModeRules.Mode.TEAM_DEATH_MATCH)
	lobby.request_team_count(2, 8)
	var competitive := Presets.apply(lobby, 2, "competitive")
	context.expect_true(competitive.ok, "semi-competitive setup accepts an existing large NPC lobby")
	context.expect_equal(lobby.player_limit, 4, "semi-competitive preset shrinks former eight-team lobby")
	context.expect_equal(lobby.config.team_count, 2, "semi-competitive preset establishes two teams")
	context.expect_equal(lobby.npc_peer_ids_view().size(), 0, "semi-competitive preset removes all NPCs")
	context.expect_true((competitive.added_npcs as Array).is_empty(), "preset cannot recreate NPC entities removed during resizing")
	context.expect_true(lobby.config.competitive_view, "semi-competitive preset enables equal view and ally cameras")
	context.expect_false(lobby.config.random_spawn_powerups, "semi-competitive preset disables pickups")
	context.expect_equal(lobby.config.arena_effects.mode, ArenaEffectRules.OFF, "semi-competitive preset disables random arena effects")
	context.expect_true(lobby.team_setup_error().is_empty(), "human participants are automatically assigned across both teams")
	...
	_validate_admin_leadership(context)
...
static func _validate_rematch(context: TestContext) -> void:
	...
	context.expect_false(bridge.can_control_results(3), "guest cannot control final results without admin authentication")
	bridge.in_game_admin.configure("results-admin-secret-2026")
	var admin_challenge := bridge.in_game_admin.begin(3, "192.0.2.3", 1.0)
	var admin_proof := NetworkProtocol.admin_password_proof(String(admin_challenge.challenge), "results-admin-secret-2026")
	context.expect_true(bridge.in_game_admin.authenticate(3, "192.0.2.3", admin_proof, 2.0).ok, "guest unlocks admin rights for results controls")
	context.expect_true(bridge.can_control_results(3), "authenticated admin can control final results without lobby leadership")
	bridge.match_coordinator.machine.state = MatchStateMachine.State.MATCH_RESULT
	bridge.commands.request_extend_match(3)
	context.expect_equal(bridge.match_coordinator.machine.state, MatchStateMachine.State.ROUND_RESULT, "authenticated admin can add five rounds from final results")
	bridge.match_coordinator.machine.state = MatchStateMachine.State.MATCH_RESULT
	bridge.commands.request_return_to_lobby(3)
	context.expect_true(bridge.match_coordinator == null, "authenticated admin can return every player to the lobby")
	bridge.in_game_admin.revoke(3)
	context.expect_false(bridge.can_control_results(3), "locking admin removes authority over final results")
	bridge.free()


static func _validate_admin_leadership(context: TestContext) -> void:
	var bridge := NetworkBridge.new()
	bridge.lobby = ServerLobby.new()
	for peer_id in [2, 3, 4]:
		bridge.lobby.admit(peer_id, "Pilot%d" % peer_id)
	context.expect_equal(bridge.lobby.leader_id, 2, "first admitted player leads when nobody is an admin")
	bridge.in_game_admin.configure("leadership-admin-secret-2026")
	for peer_id in [3, 4]:
		var challenge := bridge.in_game_admin.begin(peer_id, "192.0.2.%d" % peer_id, 1.0)
		var proof := NetworkProtocol.admin_password_proof(String(challenge.challenge), "leadership-admin-secret-2026")
		context.expect_true(bridge.in_game_admin.authenticate(peer_id, "192.0.2.%d" % peer_id, proof, 2.0).ok, "connected player can unlock admin leadership")
		bridge._elect_lobby_leader()
		context.expect_equal(bridge.lobby.leader_id, 3, "earliest joined unlocked admin leads the lobby")
	bridge.in_game_admin.revoke(3)
	bridge._elect_lobby_leader()
	context.expect_equal(bridge.lobby.leader_id, 4, "locking the lead admin transfers control to another unlocked admin")
	bridge.in_game_admin.revoke(4)
	bridge._elect_lobby_leader()
	context.expect_equal(bridge.lobby.leader_id, 2, "locking the last admin returns leadership to the first joiner")
	var reconnect_challenge := bridge.in_game_admin.begin(4, "192.0.2.4", 3.0)
	var reconnect_proof := NetworkProtocol.admin_password_proof(String(reconnect_challenge.challenge), "leadership-admin-secret-2026")
	bridge.in_game_admin.authenticate(4, "192.0.2.4", reconnect_proof, 4.0)
	bridge._elect_lobby_leader()
	context.expect_true(bool(bridge._serialized_lobby_state().leader_is_admin), "lobby state identifies an unlocked admin leader")
	bridge._on_session_peer_departed(4)
	context.expect_equal(bridge.lobby.leader_id, 2, "disconnecting admin restores first-joiner leadership")
	context.expect_false(bool(bridge._serialized_lobby_state().leader_is_admin), "lobby state clears the admin leader label after departure")
	bridge.lobby.remove(2)
	bridge._elect_lobby_leader()
	context.expect_equal(bridge.lobby.leader_id, 3, "leader departure chooses the earliest remaining human")
	bridge.free()
