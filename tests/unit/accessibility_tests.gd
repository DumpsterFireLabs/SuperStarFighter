...
static func run(context: TestContext, parent: Node) -> void:
	...
	context.expect_approx(client.network_world.hud_camera.camera_shake_remaining, 0.0, "disabled shake cannot start from combat feedback")
	context.expect_approx(client.network_world.hud_camera.camera_kick_remaining, 0.0, "disabled shake also suppresses boost kick")
	_press_pad(client.get_viewport(), JOY_BUTTON_DPAD_DOWN)
	...
	_press_pad(client.get_viewport(), JOY_BUTTON_A)
	context.expect_true(client.network_world.replicated_visuals.effects_layer.reduced_flashes, "controller flash preference reaches combat effects")
	var ship: CombatShipView = client.network_world.replicated_visuals._ensure_ship(1, {"position": Vector2.ZERO})
	...
	client.network_world.replicated_visuals.effects_layer.spawn_damage(Vector2.ZERO, Vector2.RIGHT)
	context.expect_equal(client.network_world.replicated_visuals.effects_layer.effects.size(), 1, "reduced flashes retains damage location cue")
	...
	context.expect_equal(client.network_world.hud_camera.hud_root.scale, Vector2(1.5, 1.5), "combat HUD text and bars scale together")
	context.expect_true(client.network_world.hud_camera.hud_panel.size.x + 16.0 + 420.0 + 24.0 < client.network_world.hud_camera.hud_root.size.x, "largest HUD leaves room for kill feed")
