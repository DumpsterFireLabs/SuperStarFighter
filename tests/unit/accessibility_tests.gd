...
static func run(context: TestContext, parent: Node) -> void:
	...
	client.accessibility_preferences.settings_path = preferences.settings_path
	...
	client.accessibility_preferences.set_values(Preferences.DEFAULTS)
	...
	client.hud_scale_control.value = 100.0
	client.reduced_shake_control.set_pressed_no_signal(false)
	client.reduced_flashes_control.set_pressed_no_signal(false)
	context.expect_equal(client.get_viewport().gui_get_focus_owner(), client.hud_scale_control, "opening accessibility gives focus to HUD size")
	_press_key(client.get_viewport(), KEY_TAB)
	context.expect_equal(client.get_viewport().gui_get_focus_owner(), client.reduced_shake_control, "real Tab input reaches shake control")
	_press_key(client.get_viewport(), KEY_TAB, true)
	context.expect_equal(client.get_viewport().gui_get_focus_owner(), client.hud_scale_control, "real Shift+Tab returns to size control")
	client.input_profiles.set_scheme(InputProfileManager.Scheme.CONTROLLER, false)
	client.settings_tabs.get_tab_bar().grab_focus()
	context.expect_equal(client.get_viewport().gui_get_focus_owner(), client.settings_tabs.get_tab_bar(), "settings tab selector takes keyboard/controller focus")
	_press_pad(client.get_viewport(), JOY_BUTTON_DPAD_LEFT)
	context.expect_equal(client.settings_tabs.current_tab, 1, "controller can move from accessibility to controls tab")
	_press_pad(client.get_viewport(), JOY_BUTTON_DPAD_RIGHT)
	context.expect_equal(client.settings_tabs.current_tab, 2, "controller can return to accessibility tab without focus being stolen")
	_press_pad(client.get_viewport(), JOY_BUTTON_DPAD_DOWN)
	context.expect_equal(client.get_viewport().gui_get_focus_owner(), client.hud_scale_control, "controller can enter selected tab's first control")
	_press_pad(client.get_viewport(), JOY_BUTTON_DPAD_RIGHT)
	context.expect_approx(client.hud_scale_control.value, 110.0, "controller D-pad adjusts focused HUD size")
	...
	context.expect_true(client.reduced_shake_control.button_pressed, "controller confirm toggles focused shake setting")
	...
	client._change_accessibility_setting("hud_scale", 1.5)
	...
	context.expect_false(client.settings_panel.visible, "controller cancel exits accessibility settings")
	client.connection_tabs.current_tab = 0
	client.connection_tabs.get_tab_bar().grab_focus()
	_press_pad(client.get_viewport(), JOY_BUTTON_DPAD_RIGHT)
	context.expect_equal(client.connection_tabs.current_tab, 1, "controller can select direct connection from the LAN tab")
	_press_pad(client.get_viewport(), JOY_BUTTON_DPAD_RIGHT)
	context.expect_equal(client.connection_tabs.current_tab, 2, "controller can select hosting from the direct connection tab")
	client.input_profiles.set_scheme(InputProfileManager.Scheme.KEYBOARD_MOUSE, false)
	...
	context.expect_true(client.offline_sandbox.visible and not client.connection_screen.visible, "lab flight remains in the current offline session")
	_press_key(client.get_viewport(), KEY_F2)
