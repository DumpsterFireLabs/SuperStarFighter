...
static func run(context: TestContext, parent: Node) -> void:
	...
	context.expect_equal(view.camera.position, pilot.position, "initial living snapshot places camera at the local spawn")
	pilot.alive = false
	pilot.health = 0.0
	_snapshot(view, world, 4)
	context.expect_equal(view.spectator_target_id, 2, "death selects the surviving spectator target")
	...
	var arena_view_position := view.camera.position
	_snapshot(view, world, 133)
	context.expect_equal(view.camera.position, arena_view_position, "dead snapshots also leave the arena camera alone")
	world.respawn_peer(1, pilot.stats, Vector2(600, 400))
	_snapshot(view, world, 136)
	context.expect_equal(view.camera.position, pilot.position, "respawn restores the camera to the new local spawn")
	context.expect_equal(view.spectator_target_id, 0, "respawn leaves spectator mode")
	...
	view.local_peer_id = 1
	pilot.alive = false
	var initial_arena_position := view.camera.position
	_snapshot(view, world, 139)
	context.expect_equal(view.camera.position, initial_arena_position, "initial dead snapshot preserves the late spectator's arena camera")
...
static func _snapshot(view: NetworkWorldView, world: AuthoritativeWorld, tick: int) -> void:
