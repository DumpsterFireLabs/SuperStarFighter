...
static func _validate_entry_and_timers(context: TestContext) -> void:
	...
	context.expect_equal(machine.state_deadline_tick, 1200, "draft deadline uses shared 20-second duration")
	context.expect_false(machine.is_draft_timed_out(1199), "draft is live before deadline")
	context.expect_true(machine.is_draft_timed_out(1200), "draft times out at server deadline")
