...
static func run(context: TestContext, tree_parent: Node) -> void:
	...
	context.expect_true(client.connection_controller.lobby.lobby_panel.custom_minimum_size.x >= 640.0, "online lobby uses the enlarged interface width")
	context.expect_true(client.draft_controller.draft_panel.custom_minimum_size.x >= 1200.0, "draft interface uses the enlarged readable width")
	context.expect_true(client.draft_controller.draft_panel.visible, "human draft panel renders when a private offer arrives")
	context.expect_true(client.draft_controller.draft_title.text.contains("30.0s"), "rendered draft countdown begins from the authoritative 30-second deadline")
	context.expect_equal(client.draft_controller.draft_buttons.size(), 5, "human draft renders all five offered cards")
	for index in client.draft_controller.draft_buttons.size():
		var button := client.draft_controller.draft_buttons[index] as Button
		...
		var rarity_label := client.draft_controller.draft_rarity_labels[index] as Label
	...
	var draft_hover := client.draft_controller.draft_buttons[0] as CardHoverButton
