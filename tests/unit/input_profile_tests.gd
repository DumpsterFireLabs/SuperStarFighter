...
static func run(context: TestContext) -> void:
	...
	context.expect_equal(manager.flight_mode, InputProfileManagerScript.FlightMode.NEWTONIAN, "Newtonian ship-facing flight remains the default")
	var relative_input := manager.movement_input_for_aim(PI * 0.5)
	context.expect_equal(relative_input, Vector2.ZERO, "idle Newtonian flight produces no movement input")
	context.expect_true(&"ui_accept" in manager.rebind_actions(), "keyboard profile exposes menu confirmation for rebinding")
	manager.set_flight_mode(InputProfileManagerScript.FlightMode.RELATIVE, false)
	...
	context.expect_equal(restored.flight_mode, InputProfileManagerScript.FlightMode.RELATIVE, "flight mode persists between launches")
