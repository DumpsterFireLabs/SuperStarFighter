...
static func _validate_card_powerups(context: TestContext) -> void:
	...
	context.expect_equal(powerup_cloak.cloak_charges_remaining, 1, "a mid-heat Cloak! pickup immediately records its stack")
...
static func _validate_new_card_mechanics(context: TestContext) -> void:
	...
	context.expect_equal(cloak_stats.cloak_capacity, 1, "Cloak! records the owned stack count")
	...
	context.expect_equal(cloaked.cloak_charges_remaining, 1, "activating Cloak! preserves unlimited uses")
	context.expect_true(cloaked.cloak_remaining > 4.9, "Cloak! retains almost its full five-second duration after activation")
	context.expect_true(cloaked.cloak_cooldown_remaining > 29.9, "activating Cloak! starts its 30-second cooldown")
	...
	cloak_world.step(1.0 / GameConstants.PHYSICS_TICKS_PER_SECOND)
	context.expect_false(cloaked.is_cloaked(), "Cloak! cannot reactivate during cooldown")
	var two_cloak_stats := StatSystem.derive({&"cloak": 2}, catalog)
	cloaked.reset_for_heat(two_cloak_stats, Vector2(400.0, 400.0))
	context.expect_equal(cloaked.cloak_charges_remaining, 2, "a new heat retains the cloak stack count")
	...
	cloaked.release_special_activation()
	cloaked.step(Vector2.ZERO, 0.0, false, two_cloak_stats.cloak_cooldown_seconds())
	context.expect_true(cloaked.activate_cloak(), "two Cloak! cards become available after 15 seconds")
	cloaked.reset_for_heat(two_cloak_stats, Vector2(400.0, 400.0))
	...
	context.expect_equal(cloaked.cloak_charges_remaining, 2, "an objective respawn retains cloak stacks")
	context.expect_true(cloaked.cloak_cooldown_remaining > 14.9, "an objective respawn does not clear the Cloak! cooldown")
