...
static func run(context: TestContext, parent: Node) -> void:
	...
	context.expect_true(client.draft_buttons[0].disabled, "confirmed selection remains locked pending authority")
	client._on_match_event(&"REQUEST_REJECTED", 2, {"message": "Try another card"})
	context.expect_false(client.draft_buttons[0].disabled, "server rejection restores actionable draft controls")
	context.expect_equal(client.active_offer_token, "owned-offer", "rejection preserves the live server offer token")
	client._select_draft_card(1)
	client._on_match_event(&"DRAFT_RESOLVED", 3, {"builds": {2: {&"heavy_rounds": 1}}})
	context.expect_equal(client.active_offer_token, "", "draft resolution clears controller offer state")
	context.expect_equal(client.pending_draft_index, -1, "draft resolution clears pending confirmation")
	context.expect_false(client.draft_panel.visible, "draft resolution closes owned surface")
	...
	context.expect_false(client.draft_panel.visible or client.results_panel.visible or client.scoreboard_panel.visible, "session navigation closes all owned match screens")
	client.free()
