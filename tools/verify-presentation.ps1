...
foreach ($resolution in @(
    ...
    foreach ($screen in @('splash', 'menu', 'host_menu', 'settings', 'lobby_32', 'lobby_npc_difficulties', 'draft', 'draft_bye', 'combat', 'scoreboard', 'spectator', 'pause', 'results', 'error')) {
...
Write-Host "Presentation verification passed: splash, LAN browser, host menu, settings, 32-player lobby, per-NPC difficulties, draft, winner draft bye, combat, live scoreboard, spectator, pause, structured results, and error screens rendered at 1280x720, 1920x1080, 2560x1080, and 3440x1440."
