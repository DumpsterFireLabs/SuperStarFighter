...
- `world_snapshot(server_tick, acknowledged_input, player_states, local_correction)` — unreliable ordered. The common records remain 35 bytes each; one 57-byte recipient trailer carries life generation, consumed special identity, weapon state, shield recovery state, ability timers, active ordnance/mine counts, and owner budget evictions. A full 32-player snapshot is 1,187 bytes.
