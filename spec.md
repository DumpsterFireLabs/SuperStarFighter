...
Each tier also maintains a progressively tighter preferred engagement band. NPCs select the nearest living target within their awareness range, periodically orbit/strafe, retreat when too close, and cannot fire while shielding. NPC fire requires an unobstructed projectile line to the target. When an obstacle blocks a two-NPC engagement, the lower-ID pilot holds and the higher-ID pilot takes a deterministic flank around the obstruction; the chosen flank side changes only after an eight-second epoch so mirrored counter-strafing cannot persist indefinitely. During the overtime warning or active shrink, every NPC—including Passive—steers toward an achievable point inside the safe radius. Boundary escape takes priority over pursuit, with the strongest priority once the NPC is taking boundary damage.
