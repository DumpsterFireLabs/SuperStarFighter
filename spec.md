...
- Use `ENetMultiplayerPeer` over UDP with protocol version `29` and a maximum of 32 admitted client peers plus eight bounded pre-admission slots. Version 29 adds host-authorized atomic preset and fresh-rematch RPCs. Version 28 introduced independently selected special inputs, recipient combat feedback, explicit mine detonation events, reserved mine budgets, and local ordnance telemetry. Binary packet version is `12`; older clients must update together with the server. Recipient-filtered cloak visibility, objective reveals, effective-benefit automatic drafting, timed heats, and reliable powerup expiry/removal remain in force.
