...
- Use `ENetMultiplayerPeer` over UDP with protocol version `13` and a maximum of 32 client peers in addition to the server. Version 13 adds configurable team counts plus explicit/automatic per-participant team selection to authoritative lobby state and control messages; it retains version 12's bounded projectile replication and independent objective-snapshot channel.
