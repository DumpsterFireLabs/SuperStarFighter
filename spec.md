...
- Use `ENetMultiplayerPeer` over UDP with protocol version `28` and a maximum of 32 admitted client peers plus eight bounded pre-admission slots. Version 28 adds independently selected special inputs, recipient combat feedback, explicit mine detonation events, reserved mine budgets, and local ordnance telemetry. Binary packet version is `12`; older clients must update together with the server. Recipient-filtered cloak visibility, objective reveals, effective-benefit automatic drafting, timed heats, and reliable powerup expiry/removal remain in force.
