...
static func _apply_clamps(stats: CombatStats) -> void:
	stats.max_health = clampf(stats.max_health, 25.0, 250.0)
	stats.max_speed = clampf(stats.max_speed, 200.0, 900.0)
	stats.acceleration = clampf(stats.acceleration, 300.0, 2000.0)
	stats.drag = clampf(stats.drag, 250.0, 2500.0)
	stats.projectile_damage = clampf(stats.projectile_damage, 5.0, 100.0)
	stats.fire_rate = clampf(stats.fire_rate, 1.0, 12.0)
	stats.magazine_size = clampi(stats.magazine_size, 1, 30)
	stats.reload_duration = clampf(stats.reload_duration, 0.35, 4.0)
	stats.projectile_speed = clampf(stats.projectile_speed, 400.0, 1800.0)
	stats.projectile_count = clampi(stats.projectile_count, 1, 3)
	stats.projectile_spread_degrees = clampf(stats.projectile_spread_degrees, 0.0, 20.0)
	stats.pierce_count = clampi(stats.pierce_count, 0, 3)
	stats.ricochet_count = clampi(stats.ricochet_count, 0, 3)
	stats.shield_capacity = clampf(stats.shield_capacity, 20.0, 250.0)
	stats.shield_regeneration = clampf(stats.shield_regeneration, 5.0, 100.0)
	stats.shield_continuous_drain = clampf(stats.shield_continuous_drain, 5.0, 100.0)
	stats.shield_regeneration_delay = clampf(stats.shield_regeneration_delay, 0.25, 4.0)
	stats.shield_arc_degrees = clampf(stats.shield_arc_degrees, 60.0, 240.0)
