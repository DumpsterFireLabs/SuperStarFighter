...
static func move_ship(position: Vector2, velocity: Vector2, delta: float) -> Dictionary:
	...
	var from_center := next_position - ArenaLayout.center()
	var central_limit := ArenaLayout.CENTRAL_RADIUS + radius
	if from_center.length_squared() < central_limit * central_limit:
		var normal := from_center.normalized() if not from_center.is_zero_approx() else Vector2.RIGHT
		next_position = ArenaLayout.center() + normal * central_limit
...
static func projectile_obstacle_normal(position: Vector2, radius: float) -> Vector2:
	...
	for rectangle in ArenaLayout.cover_rectangles():
		...
		if expanded.has_point(position):
			...
			return Vector2(signf(local.x), 0.0) if absf(normalized.x) > absf(normalized.y) else Vector2(0.0, signf(local.y))
	var from_center := position - ArenaLayout.center()
	if from_center.length() <= ArenaLayout.CENTRAL_RADIUS + radius:
