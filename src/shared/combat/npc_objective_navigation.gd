...
func steering(peer_id: int, from: Vector2, destination: Vector2, map_id: StringName, tick: int) -> Vector2:
	if from.distance_to(destination) <= GameConstants.SHIP_COLLISION_RADIUS * 1.25:
		_routes.erase(peer_id)
		return Vector2.ZERO
	if segment_clear(from, destination, map_id):
		_routes.erase(peer_id)
	...
	var stale: bool = route.is_empty() or route.get("map", &"") != map_id or tick >= int(route.get("refresh", 0)) or (route.get("destination", destination) as Vector2).distance_to(destination) > 320.0
	if stale and _plans_this_tick < MAX_PLANS_PER_TICK:
		_plans_this_tick += 1
		route = {"map": map_id, "refresh": tick + ROUTE_REFRESH_TICKS + posmod(peer_id, 17), "destination": destination, "points": plan_route(from, destination, map_id)}
	...
	if not segment_clear(from, points[0], map_id, GameConstants.SHIP_COLLISION_RADIUS + 2.0):
		route.refresh = tick
		return Vector2.ZERO
	return (points[0] - from).limit_length(220.0) / 220.0


static func segment_clear(from: Vector2, to: Vector2, map_id: StringName, clearance: float = CLEARANCE - 2.0) -> bool:
...
static func _point_clear(point: Vector2, map_id: StringName) -> bool:
...
static func _graph(map_id: StringName) -> Dictionary:
	if _graphs.has(map_id):
	...
	for first in points.size():
		for second in range(first + 1, points.size()):
			if segment_clear(points[first], points[second], map_id):
	...
	_graphs[map_id] = graph
	return graph


static func plan_route(from: Vector2, destination: Vector2, map_id: StringName) -> PackedVector2Array:
	if segment_clear(from, destination, map_id):
		return PackedVector2Array([destination])
	var graph := _graph(map_id)
	...
	distances.fill(INF)
	parents.fill(-1)
	var start_clearance := CLEARANCE - 2.0 if _point_clear(from, map_id) else GameConstants.SHIP_COLLISION_RADIUS + 2.0
	for index in count:
		if segment_clear(from, points[index], map_id, start_clearance):
	...
	for _iteration in count:
		...
		if current < 0 or shortest >= best_distance:
			break
		visited[current] = 1
		if segment_clear(points[current], destination, map_id):
