...
func _capture_sequence() -> void:
	...
	client.draft_controller.show_draft_offer({"offer_token": "capture", "card_ids": [&"phase_thrusters", &"blink_capacitor", &"beam_emitter", &"prismatic_lance", &"zero_point_loader"], "deadline_tick": 1800})
	...
	client.match_state.update_fields({"builds": {2: {&"twin_shot": 5, &"heavy_rounds": 2}}})
	client.draft_controller.show_draft_offer({"offer_token": "capture-cap", "card_ids": [&"twin_shot", &"phase_thrusters", &"beam_emitter", &"prismatic_lance", &"zero_point_loader"], "deadline_tick": 1800})
	...
	client.match_state.update_fields({"draft_bye_peer_id": 2})
	...
	client.match_state.update_fields({"competitive_view": true})
	client.network_world.apply_match_state(client.latest_match_payload)
	client._update_match_presentation()
	await process_frame
	await process_frame
	...
	client.match_state.update_fields({"competitive_view": false})
	client.network_world.apply_match_state(client.latest_match_payload)
	await process_frame
	...
	client.match_state.update_fields({"game_mode": GameModeRules.Mode.TEAM_DEATH_MATCH})
	client.match_state.update_fields({"game_mode_name": "Team Death Match"})
	client.match_state.update_fields({"teams": {2: 1, 3: 2, 4: 1, 5: 8}})
	client.match_state.update_fields({"objective": {}})
	...
	for state in team_states:
		(client.network_world.replicated_visuals.ships[int(state.peer_id)] as CombatShipView).global_position = state.position
	var team_registry := client.network_world.replicated_visuals.authoritative_projectiles as ProjectileRegistry
	for id in range(1, 9):
	...
	client.network_world.replicated_visuals.projectile_layer.queue_redraw()
	client.network_world.hud_camera.indicator_layer.queue_redraw()
	client._update_match_presentation()
	...
	client.network_world.local_prediction.selected_special_slot = 2
	client.network_world.local_prediction.local_mine_charges_remaining = 8
	client.network_world.local_prediction.local_missile_charges_remaining = 18
	client.network_world.local_prediction.local_cloak_charges_remaining = 1
	...
	client._apply_accessibility_settings()
	client.network_world.local_prediction.action_latch.active = {&"fire": true, &"shield": false}
	client.network_world.hud_camera._update_diagnostics()
	...
	client.network_world.apply_accessibility_settings({"hud_scale": 1.0, "reduced_shake": false, "reduced_flashes": false, "constrain_hud": true})
	client.network_world.hud_camera.combat_feedback_panel.clear_feedback()
	client.network_world.apply_builds(client.latest_match_payload.get("builds", {}))
	client.match_state.update_fields({"game_mode": GameModeRules.Mode.CAPTURE_THE_FLAG})
	client.match_state.update_fields({"game_mode_name": "Capture the Flag"})
	client.match_state.update_fields({"objective": {"active": true, "mode": GameModeRules.Mode.CAPTURE_THE_FLAG, "flag_position": Vector2(3000, 1600), "flag_carrier_id": 3, "capture_zones": {2: Vector2(2800, 1600), 3: Vector2(180, 300)}}})
	client.network_world.apply_match_state(client.latest_match_payload)
	client._update_match_presentation()
	await _capture(client, "flag_navigation")
	client.match_state.update_fields({"objective": client.latest_match_payload.get("objective", {}).merged({"flag_carrier_id": 2}, true)})
	client.network_world.apply_match_state(client.latest_match_payload)
	await _capture(client, "flag_return")
	client.match_state.update_fields({"game_mode": GameModeRules.Mode.KING_OF_THE_HILL})
	client.match_state.update_fields({"game_mode_name": "King of the Hill"})
	client.match_state.update_fields({"objective": {"active": true, "mode": GameModeRules.Mode.KING_OF_THE_HILL, "position": Vector2(700, 430), "controller_id": 3, "progress": {3: 12.5}, "target_seconds": 20.0}})
	client.network_world.apply_match_state(client.latest_match_payload)
	client._update_match_presentation()
	await _capture(client, "hill_enemy")
	client.match_state.update_fields({"objective": client.latest_match_payload.get("objective", {}).merged({"controller_id": 0}, true)})
	...
	client.network_world.replicated_visuals.projectile_layer.queue_redraw()
	client.network_world.set_physics_process(false)
	var afterburner_ship := client.network_world.replicated_visuals.ships[2] as CombatShipView
	...
	client.network_world.replicated_visuals.effects_layer.spawn_mine_explosion(Vector2(760.0, 300.0))
	client.network_world.replicated_visuals.effects_layer._process(CombatEffectsLayer.MINE_EFFECT_DELAY + 0.08)
	await _capture(client, "mine_explosion")
	client.network_world.replicated_visuals.effects_layer.clear_effects()
	for map_id in ArenaLayout.map_ids():
		client.match_state.update_fields({"map_id": map_id})
		client.match_state.update_fields({"map_name": ArenaLayout.display_name(map_id)})
		client.network_world.apply_match_state(client.latest_match_payload)
		client._update_match_presentation()
		await _capture(client, "map_%s" % map_id)
	client.match_state.update_fields({"map_id": &"solar_tide"})
	client.match_state.update_fields({"map_name": ArenaLayout.display_name(&"solar_tide")})
	client.network_world.apply_match_state(client.latest_match_payload)
	client._update_match_presentation()
	client.network_world.set_physics_process(false)
	client.network_world.hud_camera.camera.position = ArenaLayout.center(&"solar_tide")
	...
	client._apply_accessibility_settings()
	client.network_world.set_physics_process(true)
	client.match_state.update_fields({"participant_peer_ids": [2, 3, 4, 5]})
	client.match_state.update_fields({"scores": {
		2: {"heat_wins": 1, "round_wins": 1},
		3: {"heat_wins": 1, "round_wins": 0},
		4: {"heat_wins": 0, "round_wins": 1},
		5: {"heat_wins": 0, "round_wins": 0},
	}})
	client.match_state.update_fields({"builds": {
	...
	client.match_state.update_fields({"alive_peer_ids": [3]})
	...
	client.match_state.update_fields({"game_mode": GameModeRules.Mode.TEAM_CAPTURE_THE_FLAG})
	client.standings_controller._results_rows_dirty = true
	client.match_state.update_fields({"objective_contributions": {2: {"flag_captures": 3, "carrier_stops": 2, "flag_carry_seconds": 46.5}, 3: {"flag_captures": 1, "carrier_stops": 1, "flag_carry_seconds": 28.0}, 4: {"flag_captures": 0, "carrier_stops": 4, "flag_carry_seconds": 19.0}}})
	client._update_match_presentation()
...
func _capture_crowded_combat(client: Node) -> void:
	...
	view.hud_camera._update_diagnostics()
	view.hud_camera.camera.position = Vector2(1600, 900)
	view.hud_camera.camera.reset_smoothing()
	view.replicated_visuals.authoritative_projectiles = ProjectileRegistry.new()
	view.replicated_visuals.projectile_layer.registry = view.replicated_visuals.authoritative_projectiles
	...
	view.replicated_visuals.projectile_layer.visible_world_rect = view.hud_camera._visible_world_rect()
	view.replicated_visuals.effects_layer.visible_world_rect = view.hud_camera._visible_world_rect()
	view.replicated_visuals.effects_layer.clear_effects()
	view.replicated_visuals.effects_layer.set_process(false)
	for index in 128:
		var position := Vector2(950 + index % 8 * 190, 600 + index % 4 * 160)
		view.replicated_visuals.effects_layer.spawn_impact(position)
		if index < 16:
			view.replicated_visuals.effects_layer.spawn_mine_explosion(position)
		if index < 32:
			view.replicated_visuals.effects_layer.spawn_rebound(position)
	view.replicated_visuals.effects_layer.spawn_damage(Vector2(1600, 900), Vector2.RIGHT, true)
	view.apply_combat_feedback({"hit_count": 3, "hit_damage": 60, "guard_count": 1, "last_guard_reason": "perfect_guard"})
	view.replicated_visuals.projectile_layer.queue_redraw()
	...
	for sample in 36:
		await process_frame
		view.replicated_visuals.projectile_layer.queue_redraw()
		view.replicated_visuals.effects_layer.queue_redraw()
		var started := Time.get_ticks_usec()
		RenderingServer.force_draw(false)
		if sample >= 6:
			samples.append(Time.get_ticks_usec() - started)
	samples.sort()
	print("SSF_CROWD_RENDER_RESULT resolution=%s draw_submit_p95_usec=%d active_projectiles=%d drawn_projectiles=%d effects=%d suppressed=%d" % [capture_label, samples[ceili(samples.size() * 0.95) - 1], view.replicated_visuals.authoritative_projectiles.size(), view.replicated_visuals.projectile_layer.last_drawn_projectiles, view.replicated_visuals.effects_layer.effects.size(), view.replicated_visuals.effects_layer.dropped_effect_count])
	...
	view.replicated_visuals.effects_layer.set_process(true)
