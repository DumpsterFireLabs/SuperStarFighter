...
- Support 2–32 total participants in free-for-all matches, consisting of human players and optional server-owned NPC opponents. A lobby leader may set the total seat limit from 2 through the server capacity (never above 32), enable NPC fill, and start alone when ready; NPC fill occupies every vacant configured seat and waiting NPCs yield seats to joining humans.
- Players move in ship-relative space with keyboard/mouse by default or an explicitly selected controller/joystick profile. Both profiles provide independent movement, aim, fire, shield, menus, draft, scoreboard, diagnostics, and spectator controls; every binding is remappable and controller deadzone is persistent.
...
- The launch catalog contains exactly 120 unlimited-stack cards across ship, shield, and weapon categories. Catalog validation rejects exact mechanical duplicates and same-stat/same-direction magnitude swaps, while 24 numeric stats plus beam and auto-repair behavior switches provide distinct stacking axes. Seven rarity tiers use visible tier weights (Common 60%, Uncommon 25%, Rare 10%, Epic 3.5%, Legendary 1.2%, Mythical 0.25%, Unobtanium 0.05%); the server rolls tiers and cards authoritatively without replacement for each offer. Multiple cards enable pulse-beam builds, including exceptional Mythical and Unobtanium transformations.
...
- Include connection, lobby, display/audio settings, configurable controls, draft, combat HUD, pilot menu, spectator, and dedicated victory screens. Persist Windowed, desktop-native Borderless Fullscreen, and selected-resolution Exclusive Fullscreen modes plus fifteen resolution choices from 1280×720 through 5120×2160, including 5120×1440 super-ultrawide. Use expand-aspect rendering so ultrawide modes reveal more arena instead of stretching it. Consolidate match state, timer, round/heat, survivors, resources, and active-profile shortcuts in a compact upper-left HUD so the arena remains unobstructed; the lobby UI is absent during the match loop.
