...
**Related documents:** [Product plan](./plan.md) · [Implementation milestones](./milestones.md) · [Player/host manual](../MANUAL.md) · [Development guide](../DEVELOPMENT.md)
...
The vertical slice does not include public matchmaking, a public internet server directory, accounts, progression between matches, chat, reconnect restoration after a game restart, cosmetic unlocks, monetization, downloadable content, manual map selection/voting, advanced map-specific hazards, anti-DDoS infrastructure, or console/mobile/web exports. It does include bounded local-subnet discovery, automatic built-in map rotation, configurable team assignment, selectable objective modes, and 90-second held seats that let a dropped player reconnect into the same match.

## 2. Terminology

- **Participant:** A human player or server-owned NPC admitted before a match starts and eligible to spawn in its heats.
- **NPC:** A server-owned participant that consumes a configured match seat but does not consume a client transport connection.
...
- Opening the pilot menu only captures local input. The lobby host may explicitly pause or resume the entire active match from that menu, including drafts, countdowns, heats, and between-round results. Global pause freezes the authoritative simulation clock, combat, NPCs, cooldowns, objectives, respawns, and phase deadlines while networking and menus remain live. Draft selections may lock during a pause; phase advancement waits for resume. All clients and late spectators receive the paused state and an intermission notice. Only the current host may resume, including after leadership transfer. Held combat actions are cleared across pause/resume.
...
| `--port=<1024-65535>` | Server/client | `7000` | Selects the gameplay TCP (WebSocket) port. |
...
Client-only visuals must not be referenced by server-only startup code. Shared arena collision and gameplay resources must remain available in dedicated-server exports. The client root coordinates screen transitions and match presentation; dedicated settings and connection/lobby controllers own their controls, mutable state, persistence and lifecycle. Keep existing capture/test accessors explicit and backed by controller-owned state. Binding capture/timeouts belong to settings; LAN discovery, embedded hosting, lobby options and appearance/password controls belong to the connection controller. This presentation-only separation does not change the network compatibility or packet versions.
...
- Every human may click their own lobby-roster appearance swatch to combine an HSV RGB colour with one of six hull patterns: Solid, Zebra Stripes, Leopard Spots, Checkerboard, Racing Stripes, or Chevrons. Other players' and NPCs' swatches are visible but not locally editable. Colour and pattern preferences persist locally, are validated and serialized by the server, and may change only while the match is inactive. Random Colour uses the server's high-contrast palette without changing the selected pattern. NPCs receive deterministic colours and patterns.
...
- If the timer expires, the server randomly selects among offered cards with an effective full-build benefit or newly enabled mechanic. NPC choices use the same candidate set, ranking effective build changes against their objective role (runner, escort, defender, or assault). If none qualify, choose from the full offer so unlimited acquisition cannot stall a draft. Manual picks and offer eligibility remain unchanged; cards without effective benefit receive an explicit warning and retain their drawbacks. If every player locks a choice early, the draft ends immediately.
...
- King of the Hill and both Capture the Flag variants respawn eliminated pilots authoritatively after a five-second countdown. Objective-mode deaths do not resolve or tie a heat. Respawns require clear geometry and ship separation inside the current safe zone; placement weighs nearby enemies, their line of sight, incoming projectile paths over 0.6 seconds, and mine blast space. Prefer the assigned base when danger is equal. Process at most two respawn attempts per tick; retry blocked spawns after 0.5 seconds and display that they are waiting for clear space. There is no added invulnerability guarantee.
...
- Random Spawn Powerups defaults off except in King of the Hill, where temporary drops default on. When enabled, the server starts a fresh leader-configured 5–90 second timer at heat unlock (20 seconds by default), then attempts one Rare-or-better card per interval at a legal position clear of obstacles, existing pickups, and living ships, fully inside the safe circle. At most eight uncollected pickups may exist. Pickups expire after 60 seconds, when overtime overtakes them, or when the heat ends; removal is reliable and never awards a card. Missed intervals do not accumulate a spawn backlog, and lack of legal space skips a spawn. A nearby living human or NPC collects a pickup authoritatively and receives the recomputed build immediately. Collected drops expire after the current heat by default; a separate default-off permanence toggle retains their stacks until match end. Tier selection uses the configured rarity weights conditioned on Rare or above. Active pickups and effective builds are included in late-join state.
...
- Solar Tide defines one static movement field, **Solar Current**, as a clockwise annulus centered on its sun. At full influence, movement aligned with the current can reach 118% of the pilot's ordinary card-modified maximum speed; a 0.16 directional input bias supplies its modest pull. Normal thrust can oppose or leave the band. The field does not damage, stun, drain shields, alter projectiles, or add synchronized runtime entities. Authority and client replay apply the same map-authored definition. The countdown names the field and direction; the arena draws its boundaries and moving arrows, with stronger boundaries in high-contrast mode.
...
- In team modes, keep personal hull cosmetics but add team-colored circular ally markers and diamond enemy markers, explicit team-number/allegiance labels, and the same shapes on ordnance and offscreen indicators. A spectator without a team sees team identity without an ally/enemy claim. Projectile color communicates current owner allegiance ahead of rarity or rebound status; free-for-all retains weapon colors. Friendly projectiles do not trigger incoming-threat warnings. Cloaked opponents remain hidden, including their team markers.
- Render Hill control with a segmented boundary, cumulative progress arc, and explicit neutral/contested/controller status. Label the local scoring zone YOUR BASE or YOUR TEAM BASE and identify other zone owners. Offscreen objectives use separate hill/flag/base shapes, direction and distance, with RETURN FLAG for the carrier and separated labels when directions overlap. Keep objective HUD text on its own line and distinguish tied cumulative leaders from a sole leader. Cache static map drawing separately from animated objectives and overtime.
...
- Projectiles ignore their owner and allied pilots. Moving projectiles do not collide with one another, but a projectile can trigger an armed mine, including its owner's mine. Free-for-all permits damage to every other pilot.
- A normal projectile is destroyed on its first ship, shield, wall, or obstacle collision. Piercing allows additional unshielded ship hits; ricochet allows wall/obstacle bounces. Every projectile and beam uses continuous swept arena collision between its prior and next positions so fast or near-tangent volley members cannot tunnel through boundaries, rectangular cover, or circular obstacles. After a bounce or piercing hit, unused travel continues within the same physics tick so the resulting path can still damage a ship before the next tick. An ordinary shield block consumes the projectile regardless of remaining pierces or bounces; Rebound Shield may reflect it and transfer ownership instead.
- A projectile cannot damage the same ship more than once. Its server record tracks already-hit peer IDs.
- Enforce 64 active projectiles per owner and 1,024 globally, including a mine allowance of 16 per owner and 512 globally within those totals. Ordinary fire replaces the oldest moving ordnance at the applicable owner/global limit and never evicts deployed mines. Deploying beyond a mine allowance replaces the oldest mine at that limit. Show authoritative local ordnance/mine counts and budget eviction feedback; cleanup, expiration, and budget replacement are not detonations.
...
- Confirm actual hull hits separately from outgoing blocked shots and the defender's shield blocks. Distinguish ordinary blocks, Perfect Guard, and Rebound. Damage totals report HP removed after resolution, excluding overkill and subsequent impacts on an already dead ship; absence of a confirmation is not a confirmed miss. Coalesce feedback per recipient and send it privately at most 20 times per second over the reliable control channel, with bounded counts and damage totals. Do not include hit-target identities, positions, directions, or projectile IDs.
- Show the lethal source and killer, including departed-owner fallback identity, self damage, and overtime. Include the authoritative relevant mechanic when known: reflection, an uncovered shield arc, shield depletion, shield-ignoring mine blast, or Shield Ram. Include victim life generation so delayed recaps cannot reappear after respawn. Disconnects and switching to spectator do not invent combat damage. Actual mine detonations emit a separate bounded visual event at their true position; projectile removal alone must never create a blast.
...
- Hill retains its larger hill-centered minimum. Flag modes retain all capture zones, including their radii, inside the minimum circle so a return trip remains possible.
...
- Every authoritative heat ends at most 60 seconds after overtime starts (105 seconds of active play by default). A normal objective completion on the deadline tick takes precedence. Otherwise Hill awards its sole connected control-time leader, including a leader awaiting respawn; compare whole earned simulation ticks, and draw ties or zero control. Unresolved deathmatch and flag modes draw. Show the synchronized remaining time during overtime and identify a timed result. This bounds each heat, not the total length of a match with repeated draws.

### 6.6 Independent Ability Selection

- The pilot selects one owned active ability from Afterburner, Mines, Missiles, or Cloak. Previous/next ability actions are remappable, defaulting to Q/E on keyboard and D-pad left/right on controller. The HUD identifies the selected ability, its activation binding, and its charge/cooldown state. Passive effects, Perfect Guard, Kinetic Vent, and Breakaway retain their own existing triggers.
- A Special press attempts only the selected ability. Preserve both the selected slot and the unique press identity across retransmissions; changing selection after pressing must not change the pending action. A repeated press identity cannot spend another charge, retry after cooldown, or reactivate after respawn. Authority checks ownership and availability; an unspecified slot resolves to the first owned ability for shared callers, never all abilities together.
...
- Use `WebSocketMultiplayerPeer` over TCP (`ws://host:port`) with a maximum of 32 admitted client peers plus eight bounded pre-admission slots, enforced by the server because WebSocket has no peer cap. Protocol version `44` replaced ENet/UDP with this transport version `45` added server-originated transport probes, and version `46` expires unanswered probes after two seconds, limits pings and probe acknowledgements separately from lobby controls (40 per second) while ignoring them from peers no longer admitted, and disconnects a peer whose server outbound buffer reaches three quarters of its capacity, and version `47` adds held-seat reconnect tokens (the `server_welcome` token argument and the `rejoin_hello` and `player_leaving` RPCs); RPC channel and transfer-mode annotations are retained but every stream is delivered reliably and in order on one TCP connection. Clients accept a hostname/IP with a port, or a `ws://`/`wss://` URL that carries its own port; `wss://` verifies the server certificate against system roots. Servers accept `--bind=<ip>` (default all interfaces) and `--behind-proxy`, which keys admission, attempt and failure limits per connection instead of per address, throttles new challenges to one per second after eight wrong passwords in 60 seconds, and refuses address bans, because Godot's WebSocket server cannot read forwarded client-address headers. The server probes each admitted peer every 250 ms; a probe round trip more than 150 ms above that peer's learned baseline (or an unanswered probe that late) marks its outbound stream congested until the excess falls below 50 ms. A congested peer receives one player snapshot in four (5 Hz), no periodic projectile corrections, and full projectile recovery at a quarter of the normal pace; reliable events, projectile deltas and objective updates are unchanged. Per-peer transport buffers are 64 KiB inbound and 512 KiB outbound on the server, and 1 MiB inbound and 64 KiB outbound on clients. Clients measure round-trip time with a 1 Hz reliable ping and retry a refused transport connect with jittered backoff (up to 12 attempts within 10 seconds) before admission. Version 31 added the authoritative missile target ID to projectile records (31 bytes) so clients can render guided flight. Version 30 added the host-authorized competitive-view RPC and replicated match policy. Version 29 added host-authorized atomic preset and fresh-rematch RPCs. Version 28 introduced independently selected special inputs, recipient combat feedback, explicit mine detonation events, reserved mine budgets, and local ordnance telemetry. Binary packet version is `13`; older clients must update together with the server. Recipient-filtered cloak visibility, objective reveals, effective-benefit automatic drafting, timed heats, and reliable powerup expiry/removal remain in force.
- Exclude cloaked player records from every other recipient, including allies and spectators. Only the owner receives the hidden ship and its correction trailer. Absence removes the client ship and interpolation/feedback history; reappearance starts from the fresh authoritative position. This withholds direct ship state, not the clues provided by public projectiles or prior visible positions. Hill occupants and flag pickup/carriers lose cloak. Missiles retain homing and mines retain proximity/magnetism against cloaked pilots; damage and collision rules are unchanged.
...
- Every projectile correction includes active Hunter Missiles, including partial corrections under heavy fire. Clients use the shared missile turn rule toward the replicated target. Predicted terrain contact or lifetime expiry stops missile visual travel while retaining its drawable until authoritative removal or correction; full snapshots recover lost removal deltas. Gameplay missiles are destroyed at their first terrain/wall impact or lifetime expiry and cannot deal subsequent damage.
...
- **Host & Join** creates an authoritative server inside the client process under an isolated `MultiplayerAPI`, then connects the playable client to it through `127.0.0.1` using the normal transport handshake. The two roles never share authority or bypass the network protocol.
- The host supplies a bounded server display name, gameplay TCP port, and required 1–64 printable-character lobby password. The fixed discovery port `7359` is reserved and cannot also be selected as the gameplay port.
- A listening server answers bounded UDP discovery queries on port `7359` with its protocol version, instance ID, display name, gameplay port, human/NPC occupancy, total-player limit, match activity, and a password-required flag. Discovery messages never carry the password, gameplay state, or authority.
...
- `client_hello(protocol_version, display_name, password_proof)` — reliable, required within 10 seconds of transport connection and only after the server's fresh challenge; the raw lobby password is never sent by this RPC.
...
- `submit_input(sequence, client_tick, move_x, move_y, aim_angle, action_bits, special_sequence, special_slot)` — unreliable ordered, encoded as one bounded binary packet.
...
- `world_snapshot(server_tick, acknowledged_input, player_states, local_correction)` — unreliable ordered. The common records remain 35 bytes each; one 63-byte recipient trailer carries life generation, consumed special identity, weapon state, shield recovery state, ability timers, active ordnance/mine counts, owner budget evictions, consumed shield press identity and elapsed input age in physics ticks. A full 32-player snapshot is 1,193 bytes.
...
- `COMBAT_FEEDBACK` through the reliable match-event RPC — private recipient accumulator drained at 20 Hz; confirmed hit/damage, outgoing block, defender guard, and optional latest death recap only.
- `mine_detonations(server_tick, events)` — unreliable ordered on the projectile-delta channel, at most eight actual detonation records per message. Events carry mine identity, owner, and position for transient blast presentation; clients do not infer a detonation from cleanup or eviction.
...
- `sequence` and ticks use wrapping unsigned 32-bit values. The server ignores duplicate or older input. Human controls become neutral after 0.5 seconds without fresh input, preserving the last aim and sequence acknowledgement. Server-owned NPC decisions do not use the human connection timeout.
- Movement components are quantized signed 16-bit values representing `[-1, 1]`; aim is quantized to an unsigned 16-bit turn; action bits contain fire, shield, manual reload, and one-shot special activation flags. Each special press has a separate wrapping uint32 identity and one selected ability slot retained across retransmissions. Only the selected owned ability is attempted once per distinct press; an old press cannot retry after its cooldown or after respawn. Input packets contain 25 bytes, including the retained shield press identity.
...
- The controlled client runs the shared combat-input simulation immediately, buffers at least 120 input frames, and restores then replays unacknowledged input after every authoritative snapshot. Replay includes actual shield activation/depletion, Afterburner impulse and duration, Breakaway timers, consumable specials, reload progress, fractional weapon cadence, and shot sequence. World damage and projectile creation remain authoritative. A changed life generation clears previous-life inputs, predicted volleys, and stale local reload state even if the death snapshot was lost.
- Sustained weapon cadence carries fractional tick remainder without banking idle or reload time. Delayed owner cleanup captures the projectiles that existed when scheduled; fresh respawn shots and projectiles reflected to another owner survive that timer.
- Draft previews compare the complete current and proposed builds after technical limits. Saturated effects are marked `AT LIMIT` on the card, in the actual before/after values, and on confirmation; any remaining drawback remains visible. Acquisition and card balance rules are unchanged.
- Correction errors up to 128 pixels are smoothed over 100 ms. Larger errors snap immediately and increment a diagnostic counter. Overlapping corrections preserve the remaining visual offset. Replay excludes elapsed ticks already covered by the authoritative held input, without treating those unacknowledged input identities as acknowledged.
...
7. **Offline Combat Lab:** A searchable card list with descriptions and rarity, add/remove stack controls, clear build, and Base Ship, Rapid Scatter, Beam Specialist, Shield Tank, and All Abilities presets. Show derived stats alongside measurements from the same `AuthoritativeWorld` used online. Configure one to five targets, 10–600 hull HP, distance from 160–900 pixels, shielding, return fire, and strafing. Editing pauses only the offline range; entering it requires fresh combat input so a UI click cannot fire a shot. Reset encounter restores ships and ordnance; reset measurements clears telemetry independently. Show shot/hull-hit/block counts, actual HP damage, DPS, and source. DPS uses active time from the first shot or hit, excludes editing pauses, and includes misses and reloads. Defeated targets remain down until reset. Provide keyboard/controller navigation and the shared selected-ability controls.

8. **Guided Introduction:** Learn to play from the main menu, or Guided introduction in the offline lab, starts seven action-based exercises using the same `AuthoritativeWorld`: move and aim, land confirmed hull damage, complete a manual reload, face and block a projectile, time a Perfect Guard, select and activate Afterburner, then choose a draft upgrade with effective stat changes. Shots that miss and merely holding a shield do not complete the corresponding exercises. Slow, low-damage practice rounds preserve normal shield arcs and the real Perfect Guard window. Show current remapped bindings, allow step retry and lesson restart, and pause practice during the draft choice. Skip/return and leaving the range restore the prior lab build and target configuration; the lesson does not overwrite a player's practice setup.
...
- Persist Accessibility preferences for combat HUD scaling from 100% to 150%, reduced shake, reduced flashes, and an optional centered 16:9 HUD safe area, enabled by default. The safe area constrains HUD placement without cropping the arena or changing gameplay zoom. Apply settings to online combat and the offline range; reduced effects retain readable shield, hit, and blast cues. Verify maximum-scale layout at 720p and ultrawide, plus keyboard/controller focus traversal in settings and the lab.
- Draft cards use dark category-tinted backgrounds with at least 85% opacity so arena action cannot overpower their text.
- Budget transient effects at 96 total, with decorative admissions limited at 48 active effects. Prefer local damage/elimination feedback to lower-priority bursts and cull offscreen effects with room for blast outlines. At 160 active projectiles, simplify redundant glow passes while retaining every in-view projectile's core shape and allegiance cues; visual culling never removes authoritative entities. Keep static map outlines subordinate to threats and objectives.
- Avoid full-screen white flashes. Screen shake is subtle, local-only, and never affects aim coordinates.
- Start with an animated splash that accepts keyboard, mouse, or controller input immediately and automatically proceeds to the connection menu after 10 seconds. The connection menu displays the canonical game version and release label. Provide one persistent settings screen from the main menu and in-match pilot menu with separate Display & Audio, Controls, and Accessibility tabs. Include persistent Windowed, Borderless Fullscreen, and Exclusive Fullscreen selection; sixteen standard, 3:2, ultrawide, and super-ultrawide resolutions through 5120×2160; Newtonian/Relative flight selection; input-profile switching; connected-controller status; controller deadzone; binding capture; and per-profile defaults. Borderless mode uses the desktop resolution, while Windowed and Exclusive Fullscreen use the saved selection. The lobby/menu must remain hidden during draft, countdown, combat, results, and spectating. Each heat countdown uses a high-contrast centered `READY` plate; it changes to `BEGIN` at 0.10 seconds remaining, persists for 0.10 seconds after unlock, and fades during that post-roll. Match completion opens a dedicated victory screen with authoritative objective contribution totals. The leader may start a fresh same-rules rematch (reset cards, scores, objectives, inventories, and presentation; retain current participants/teams), extend play with builds and scores retained, or return everyone to the lobby. Late spectators become eligible through the lobby-return path. Returning to the same connected lobby clears match-only renderer state without discarding the local peer identity, monotonic input sequence, or client tick required for prediction and server input acceptance in a rematch.
...
- Discover music paths at startup, load the active context asynchronously, and release inactive music buffers. Bound synthesized weapon variants to 96 least-recently-used entries. Retain a fixed 24-voice effects pool, with at most eight remote weapon voices and six remote effect voices; local damage/shield and objective feedback can replace lower-priority voices. Pan remote sources by horizontal listener-relative direction, preserve signal in both ears, and briefly duck music by up to 7 dB for important cues. Limit final output, while preserving independent user volume/mute controls.
- Objective audio distinguishes friendly pickup/control gain, enemy pickup/control loss, and neutral flag drops. Establish objective snapshots without replaying transition sounds on join/state changes; clear deduplication and objective baselines at match reset. Human recognition/masking acceptance remains separate from bus recordings and level analysis.
- Draft and standings controllers own their UI construction, state, and reset; root client code owns navigation/session coordination. Construct one scoreboard and one results overlay per client. Repeated result updates preserve keyboard focus and reuse unchanged rows.
...
- Every 10 seconds during a match, log connected peers, participant/entity counts, mean/p95/p99/maximum server callback duration, active-combat sample count and p95/p99, phase means, projectile budget evictions, over-budget tick count/percentage, outbound byte counts, static memory, object/node counts, and orphan-node count. The callback measurement covers admission processing, NPC/world simulation, coordination, encoding, and RPC enqueue; it excludes engine multiplayer polling outside the callback, OS transmission, rendering, and periodic metric-log work. Legacy `*_simulation_usec` fields report this expanded callback scope. Separate phase means report world/NPC, coordination, and replication time.
- A debug-only client overlay shows FPS, round-trip time and variance, snapshot arrival jitter and gaps, interpolation delay and extrapolation rate, reconciliation error/snaps, buffered input count, expired predicted shots, and the last acknowledged input.
...
- **Presentation and training tests:** Exercise action-based tutorial progress and restoration of lab settings, mechanic-family identities and effective draft highlights, settings/lobby focus and controller navigation, bounded effect priorities, and preservation of projectile counts under simplified rendering.
...
- Launch one exported or headless server and 32 scripted protocol clients through the real TCP paths.
...
- The real-client soak must observe active-combat samples and keep both active-combat and all-callback p95 below 16,667 microseconds on the development machine. These measurements include coordination and replication, so inactive periods cannot hide costly combat frames. Projectile threats and armed mines use bounded spatial queries. Pre-expand ship sweep cells for supported ordinary projectile/missile radii so short queries can borrow candidate arrays; retain the general padded query for larger radii and unchanged narrow-phase collision checks for every candidate. The overload gate separately exercises 32 Insane NPCs with 1,024 projectiles and a 512-armed-mine/512-projectile mixed case. `run-performance-benchmark.ps1 -Map <map_id>` selects the layout and reports collision subphase timings plus sweep/candidate counts. Its isolated world workload supplements, rather than replaces, real-client callback acceptance.
...
- UI remains usable at 1280×720, 1920×1080, 2560×1080, and 3440×1440 with 32 listed players, without nonuniform stretching. Presentation capture additionally covers 2880×1920 and 5120×1440, including a 32-player/1,024-projectile mixed-effects fixture and maximum-scale reduced-effects view. Its `draw_submit_p95_usec` measures warmed-up forced-render submission only, excluding the rest of the frame and GPU completion; do not interpret it as end-to-end FPS.
...
- README covering controls, local hosting, direct-IP joining, TCP port forwarding, tests, exports, known limitations, and troubleshooting.
...
### Review follow-up presentation and measurements

The interface enforces 21 logical-pixel text minimums except deliberately hidden custom-card native labels. High contrast strengthens text/panel/world boundaries. Fire and shield independently support persistent hold/toggle settings with visible latch status; menu/death/session transitions reset latches and require release before rearming. Settings and setup lists scroll and follow focus; larger wrapped draft cards retain capped-stat warnings and confirmation controls at 720p.

Stat-derived ship modules distinguish seven equipment families without changing collision or allegiance markers. Five quiet material families and numbered landmarks occupy existing collision cover only. Static material drawing remains cached.

Authoritative match observations keep 128 completed heat records with contact, eliminated time, actual drafting, turnaround, builds, byes, and pickup cohorts. Server logs emit bounded summaries and participant rows; study JSON retains complete records. The repeatable bot study informs decisions but does not substitute for human counterplay or fairness acceptance. See `docs/archive/GAMEPLAY-STUDY-2026-09-03.md` for the dated evidence and limitations.

### Competitive combat viewport

Casual matches retain expanded world visibility on wider or taller displays. The lobby leader can enable **Competitive view** in Match Setup before starting. The server freezes this policy with the match configuration, replicates it to participants and late spectators, and preserves it for fresh rematches. Changing it clears human readiness; no host or operator may change it midmatch. Dedicated hosts may pass `--competitive-view` (PowerShell launcher `-CompetitiveView`) or set the boolean `competitive_view` through the existing authenticated operator interface.

Standard clients use a single native 1920×1080 logical canvas with aspect preservation during competitive matches. Wider or taller windows receive bars and expose equal world extents at camera zoom 1; resizing does not expand the combat view. Cursor and mouse aim share a constrained logical point; menus restore the previous expanded canvas on leaving the session. This adds no secondary render target. It is display parity, not a server-side information restriction or custom-client anti-cheat boundary.

### Shared metadata, maps, and state ownership

Shipping builds use explicit selected-resource allowlists before bytecode compilation. Client packages exclude tests, development tools and reports; dedicated-server packages additionally exclude client scenes/scripts, audio and branding. Both retain shared rules, all cards and all ten map definitions. Shipping bootstrap rejects unavailable test/bot modes; the dedicated-server feature selects headless server startup. Package auditing checks actual entries and remap targets. Engine/component notices accompany distributed artifacts.

Local ability delivery repeats the original press identity and selected slot in ordinary input samples until an authoritative private correction reports that identity consumed, or a 1.25-second maximum retry window expires. Input-sequence acknowledgements alone cannot cancel pending ability delivery. Existing authority deduplication prevents repeated spending. Countdown, blocked controls, death/revival and session reset cancel pending delivery. This changes no RPC or packet layout; compatibility remains 30 and binary packets 12.

- `StatMetadata` provides the canonical numeric-stat names, display units, bounds, integer classification and benefit polarity, including the shield-capacity-dependent depletion threshold. `StatSystem` applies those same guardrails; effective values remain unchanged. Card previews and lab summaries format via the same descriptors, and share nominal modifier rows. `StatChange` is the typed production comparison contract; dictionary comparisons remain compatibility adapters.
- Ten `MapDefinition` resources under `resources/maps` define the shipped roster. `MapRegistry` validates identity, finite in-bounds positive geometry, materials and all 32 safe/spaced spawn anchors before caching read-only geometry/palette views. `ArenaLayout` retains queries and fallback semantics. Explicit resource anchors preserve original full precision. Resources introduce no dynamic hazards or new gameplay mechanics.
- `NetworkLocalPrediction`, `NetworkReplicatedVisuals`, and `NetworkHudCamera` own their respective client state and teardown. `NetworkWorldView` owns lifecycle and snapshot/match ordering. Shared drawing nodes live in the presentation namespace. `NetworkSessionOwner` and `NetworkReplicationScheduler` separate transport/admission state from replication scheduling, while `NetworkBridge` retains all RPC validation and outer callback timing.
- `HillModeHandler` and `FlagModeHandler` own typed objective state; the coordinator owns heat transitions. Typed transition snapshots preserve intermediate states, and typed match events serialize at the existing boundary. General payload contents remain dictionaries where compatibility requires them. Network format changes require explicit protocol versioning and codec validation.
