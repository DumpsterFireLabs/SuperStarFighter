...
func _run() -> void:
	...
	if objectives_only:
		report = JSON.parse_string(FileAccess.get_file_as_string("res://docs/archive/card-balance-validation-2026-09-07.json"))
		report.objectives = []
	catalog = current
	...
	for row in report.range_checks:
		if row.hit != row.expected_hit:
			...
			quit(3)
			return
	var file := FileAccess.open("res://docs/archive/card-balance-validation-2026-09-07.json", FileAccess.WRITE)
	if file == null:
		quit(2)
		return
