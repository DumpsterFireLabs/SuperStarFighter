...
This guide is for contributors working on the Godot source project. For gameplay and hosting, use the [Player and Host Manual](./MANUAL.md). For exact observable requirements, use the [authoritative specification](design/spec.md).
...
TCP never drops a stale snapshot; it queues it in the operating system's send buffer, where the application cannot see it. `NetworkSessionOwner` therefore probes each admitted peer every 250 ms (`transport_probe`/`transport_probe_ack`) and treats round trip above the peer's learned baseline as queueing. Above 150 ms of queueing (exit below 50 ms) the peer joins `congested_peers()`, and `NetworkReplicationScheduler` sends it one snapshot in four, skips periodic projectile corrections and quarters recovery pacing until it drains. Reliable events and projectile deltas are never thinned. On an 8 KB/s link with 256 projectiles this cut the slowest client's snapshot age from 35–78 ticks to a typical 15 without starving it; healthy 100 ms ± 35 ms links never trigger it. Transport buffers are sized by role (server 64 KiB in / 512 KiB out per peer, client 1 MiB in / 64 KiB out), which reduced 32-peer server memory from about 104 MB to about 13 MB. An unanswered probe expires after two seconds, so a dropped acknowledgement cannot hold a peer in congestion; newer outstanding probes still signal a real backlog. Pings and probe acknowledgements pass through `accept_transport_message`, which ignores peers that are no longer admitted (for example during an eject's disconnect grace) and limits them to 40 per second apart from lobby controls. Because a full WebSocket outbound buffer silently drops reliable RPCs, the server disconnects a peer once its buffered outbound bytes reach three quarters of the 512 KiB buffer.
