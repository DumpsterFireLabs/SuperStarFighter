...
Shield block and break cues come from bounded authoritative impact and depletion counts in the reliable `COMBAT_FEEDBACK` payload. Holding a shield or crossing its regeneration unlock threshold produces neither cue. Counts coalesce at the existing 20 Hz feedback cadence; public shield cues follow snapshot cloak visibility, while hit/guard totals remain recipient-private. Clients suppress duplicate, previous-heat, and more-than-one-second-old cues; newly visible or revived ships discard earlier cues. Offline practice drains the same shield events. Compatibility version 33 and binary packet version 14 require matching clients and servers. Player snapshots remain below the 1,200-byte budget at 1,191 bytes for 32 players.
